using System;
using System.Collections.Generic;
using System.Linq;
using System.IO;
using Newtonsoft.Json;
using Dashboard.Models.Structure;

using HostingEnv = System.Web.Hosting.HostingEnvironment;

namespace Dashboard.Models
{
    /// <summary>
    /// Загрузчик структуры панели администратора
    /// </summary>
    public class DashboardStructureLoader
    {
        // Путь к каталогу хранения разметки
        private readonly string storagePath;

        /// <summary>
        /// Загрузить разметку панели админа из файла
        /// </summary>
        /// <param name="panelName">Название панели для загрузки</param>
        /// <returns>Структура загруженной панели</returns>
        public IList<DashboardPage> LoadFromFile(string panelName)
        {
            string filePath = $"{storagePath}/{panelName}.json";
            string contents = File.ReadAllText(filePath);

            var panelConv = new PanelStructureConverter();
            var pages = JsonConvert.DeserializeObject<IList<DashboardPage>>(contents, panelConv);

            // Загрузить данные и посчитать ширину
            foreach(var page in pages)
            {
                page.ComputeWidths();
                page.LoadData();
            }

            return pages;
        }

        public DashboardStructureLoader()
        {
            storagePath = StorageConfig.StoragePath;
        }

        public DashboardStructureLoader(string storageRealPath)
        {
            storagePath = storageRealPath;
        }
    }
}